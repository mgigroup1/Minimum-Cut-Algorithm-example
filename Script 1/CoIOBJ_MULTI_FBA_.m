...
% load Cac_Clj_MulTi_FBA_example_data
model = modelJoint;
...
% [concentration,~,~] = xlsread('TABLE S1.xlsx','Fermentation Profiles','A2:J7');
...
%     end
    ...
    modelJoint.lb(5012) = data(k,8); %but
...
% multi-objective function derived from the criteria, normalized edge density of the proposed method
      if k < 65 % CoI=25
       f=[zeros(4984,1);0.15;0;0.63; 0 ;zeros(22,1);0.08;0;zeros(21,1);0.15;zeros(107,1)]';       
      ...
      elseif k >= 65  && k< 110  
       ...
       modelJoint.lb(5034) = data(k,9); %ethanol
       modelJoint.lb(5011) = data(k,7); %butanol
       modelJoint.lb(5012) = data(k,8); %butyrate
       modelJoint.lb(4988) = data(k,10); %acetone  
       
      elseif k >= 110  && k< 140   % CoI=119
       f=[zeros(4984,1);0.43;0;0.20; 0 ;zeros(22,1);0.15;0;zeros(21,1);0.21;zeros(107,1)]';       
       modelJoint.lb(4985) = data(k,5);  %isopropanol
       modelJoint.lb(4987) = data(k,4);  %acetate
       modelJoint.lb(5034) = data(k,9);  %ethanol
       modelJoint.lb(5011) = data(k,7);  %butanol
       modelJoint.lb(5012) = data(k,8);  %butyrate
       modelJoint.ub(4988) = data(k,10); %acetone  ****
      ...
      elseif k >= 140  && k< 170   
       ...
       modelJoint.lb(5011) = data(k,7);  %butanol
       modelJoint.ub(4988) = data(k,10); %acetone    
       modelJoint.ub(5012) =  data(k,8); %butyrate
    
       elseif k >= 170 && k<210    % CoI=198
       f=[zeros(4984,1);0.25;0;0.28; 0 ;zeros(22,1);0.23;0;zeros(21,1);0.24;zeros(107,1)]';       
       modelJoint.lb(4985) = data(k,5);  %isopropanol
       modelJoint.lb(4987) = data(k,4);  %acetate
       modelJoint.lb(5034) = data(k,9);  %ethanol
       modelJoint.lb(5011) = data(k,7);  %butanol
       ...
       elseif k >= 210  && k<270   % CoI=250
       f=[zeros(4984,1);0.19;0;0.34; 0 ;zeros(22,1);0.26;0;zeros(21,1);0.20;zeros(107,1)]';       
       modelJoint.lb(4985) = data(k,5);  %isopropanol
       modelJoint.lb(4987) = data(k,4);  %acetate
       modelJoint.lb(5034) = data(k,9);  %ethanol
       modelJoint.lb(5011) = data(k,7);  %butanol 
       ...
       elseif k >= 270 && k<310    % CoI=303
       f=[zeros(4984,1);0.24;0;0.16; 0 ;zeros(22,1);0.38;0;zeros(21,1);0.22;zeros(107,1)]';       
       modelJoint.lb(4985) = data(k,5);  %isopropanol
       modelJoint.lb(4987) = data(k,4);  %acetate
       modelJoint.lb(5034) = data(k,9);  %ethanol
       modelJoint.lb(5011) = data(k,7);  %butanol 
       ...
       f=[zeros(4984,1);0.56;0;0.12; 0 ;zeros(22,1);0.21;0;zeros(21,1);0.11;zeros(107,1)]';       
       modelJoint.lb(4985) = data(k,5);   %isopropanol
       modelJoint.lb(4987) = data(k,4);   %acetate
       modelJoint.lb(5034) = data(k,9);   %ethanol
       modelJoint.lb(5011) = data(k,7);   %butanol
...
%    FBA = optimizeCbModel (modelJoint, 'max');
    ...
    if ~isempty( FBA_multi )
        ...
        store_obj(k,3) =  FBA_multi (4991); %acoitn
        store_obj(k,5) =  FBA_multi (5012); %butyrate
        store_obj(k,7) =  FBA_multi (4988); %acetone
...
subplot(3,2,5)
plot(concentration(1:5,1),concentration(1:5,9)*0.05808,'*')
hold on
xs=linspace(0,0.1*(k),k);
plot(xs,Cstore_GL(1:k,7))
title('acetone')
xlabel('Time (hr)')
ylabel('Titer (g/L)')
...
subplot(3,2,6)
hold on 
plot(concentration(1:5,1),concentration(1:5,8)*0.08701,'*')
xs=linspace(0,0.1*(k),k);
plot(xs,Cstore_GL(1:k,5))
title('butyrate')
xlabel('Time (hr)')
ylabel('Titer (g/L)')
