...
% load experinmental data package for an example./ or import the below data
% list
...
%% DMMM model with Single-objective in the second stage of optimization
...
for k = 1:498
    ...
    if k <= 119  %119
        rxnsList = {'ExCom_ac[u]'}; % ExCom_but[u] 
    elseif k > 119 && k<313
        rxnsList = {'ExCom_btoh[u]'}; % ExCom_lac__L[u]
    else
        rxnsList = {'ExCom_btoh[u]'}; % ExCom_lac__L[u]
    end
    ...
    if ~isempty(FBA.f)
        ...
        store_obj(k,1) = FBA.v(4987); %acetate
        store_obj(k,2) = FBA.v(4985); %isopropanol
        store_obj(k,3) = FBA.v(4991); %acetoin
        store_obj(k,4) = FBA.v(5011); %butanol
        store_obj(k,5) = FBA.v(5012); %butyrate
        store_obj(k,6) = FBA.v(5034); %ethanol
        store_obj(k,7) = FBA.v(4988); %acetone
        if k == 1
